...
public:
	...
	~Organism();
	//"the = 0" below indicates pure virtual functions
...
World::~World()
{
	// You have to release any allocated memory properly
...
void Ant::move()
{
	...
	// Fill in your code to move right
	else
	{
	  //(1)
	  //Move to new spot  and then
...
void Ant::breed()
{
	...
	if (breedTicks == ANTBREED)
	{
		...
		if ((y>0) && (world->getAt(x,y-1)==NULL))   //above
		{
			Ant *newAnt = new Ant(world, x, y-1);
		}
		else if ((y<WORLDSIZE-1) && (world->getAt(x,y+1)==NULL))  //down
		{
			Ant *newAnt = new Ant(world, x, y+1);
		}
		else if ((x>0) && (world->getAt(x-1,y)==NULL))  //left
		{
			Ant *newAnt = new Ant(world, x-1, y);
		}
		else if ((x<WORLDSIZE-1) && (world->getAt(x+1,y)==NULL))/*(3) condition to move right*/  //right
		{
		    Ant *newAnt = new Ant(world, x+1, y); //code to move right //(4)
		}
	}
}
...
class Doodlebug : public Organism
{
 ...
 public:
  ...
  //missing the delcarations of some virtual functions!
  //(5)
    void breed();
	void move();
	int getType();
